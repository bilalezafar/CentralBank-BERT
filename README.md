...
| `8-CBDC-Analysis and Results` | End-to-end notebook (**CBDC Analysis and Results.ipynb**) that reads **cbdc-dataset-final.csv**, runs all CBDC classifiers to create **cbdc-dataset-predictions.csv**, and generates all manuscript tables and figures; outputs saved to **prediction\_results/** and **descriptive\_results/**. The folder also contains the encoder benchmark notebook (CBDC classifiers benchmark with BERT-base.ipynb), which reproduces the matched comparison between BERT-base-uncased and CentralBank-BERT for the CBDC gate, Type, Stance, Sentiment, and Discourse, with outputs inside the notebook.|
